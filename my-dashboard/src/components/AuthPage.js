  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isRegister) {
        ...
        // Generate a clean user id automatically based on timestamp or email prefix
        const generatedId = 'U' + Math.floor(100 + Math.random() * 900);
        payload = {
          user_id: generatedId,
          name: formData.name,
      ...
      if (response.ok) {
        // Save user profile to local storage so frontend can reference user_id if needed
        if (data.user) {
